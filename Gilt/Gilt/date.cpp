...
double Date::addMonths(int m)
{
	...
	return m_serial = Date(day, newMonth, year).m_serial;
}

double Date::addYears(int y)
{
	unsigned int day = 0, month = 0, year = 0;
	DMY(day, month, year);
	...
	return m_serial = Date(day, month, newYear).m_serial;
}
